    ...
    if(isEditing){
        return(
            <div className='bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md border-2 border-blue-500 transition-all'>
                <input type='text' disabled={isSaving} value={editTitle} onChange={(e)=>setEditTitle(e.target.value)} className='w-full p-2 mb-2 border rounded-lg dark:bg-gray-700 dark:text-white dark:border-gray-600 focus:ring-2 focus:ring-blue-500 outline-none' placeholder='Todo Title' aria-label='Edit Title'/>
                ...
                <div className='flex space-x-2'>
                    <button onClick={handleUpdate} disabled={isSaving} className='px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-medium rounded-lg transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed'>{isSaving?'Saving...':'Save'}</button>
    ...
    return(
        <div className='bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md hover:shadow-lg transition-all group'>
            <div className='flex items-start space-x-3'>
                <input type='checkbox' disabled={isSaving} checked={todo.completed} onChange={handleToggleComplete} className='mt-1.5 w-5 h-5 cursor-pointer text-blue-600 rounded focus:ring-2 focus:ring-blue-500 transition-transform active:scale-125'/>
                ...
                <div className='flex space-x-1'>
                    ...
                    <button onClick={handleDelete} disabled={isSaving} className='p-2 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-all' aria-label='Delete'>
                        <svg className='w-5 h-5' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
